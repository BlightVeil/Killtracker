    ...
    def post_heartbeat(self) -> None:
        ...
        while self.heartbeat_status["active"]:
            try:
                ...
                if not self.api_key["value"]:
                    self.log.warning("Error: heartbeat will not be sent because the API key does not exist.")
                ...
                # self.log.debug(f"Sending heartbeat: {heartbeart_base}")
                response = requests.post(
                    url, 
                    headers=headers, 
                ...
                # self.log.debug(f"Received response for heartbeat: {response_data}")
