...
def setup_game_running_gui(app, rsi_name):
    ...
    connect_button = tk.Button(
        ...
        font=("Times New Roman", 12),
        command=lambda: start_heartbeat_thread(rsi_name, logger),  # Pass rsi_name and logger here
        bg="#000000",
        fg="#ffffff",
    )
...
def setup_gui(game_running):
    ...
    if game_running:
        ...
        if log_file_location:
            ...
            if rsi_handle:
                logger = setup_game_running_gui(app, rsi_handle)  # Pass rsi_handle here
...
if game_running:
    ...
    if log_file_location:
        rsi_handle = find_rsi_handle(log_file_location)
        if rsi_handle:
            logger = setup_game_running_gui(app, rsi_handle)  # Pass the RSI handle
            start_tail_log_thread(log_file_location, rsi_handle, logger)
